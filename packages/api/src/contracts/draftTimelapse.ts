...
export const draftTimelapseRouterContract = {
    ...
    create: contract("POST", "/draftTimelapse/create")
        ...
        .input(z.object({
            ...
            thumbnail: z.base64().max(2 ** 15)
                .describe("A Base64-encoded WebP data URI for the thumbnail. This thumbnail will *not* be used when publishing."),

            sessions: z.array(
                z.object({
                    fileSize: z.number()
                        .describe("The size, in bytes, of this session.")
                })
            ).describe(`

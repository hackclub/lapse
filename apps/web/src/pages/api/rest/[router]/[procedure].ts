...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!params.success)
        return res.status(400).json({ error: "invalid_request", error_description: "Missing router or procedure." });

    const { router, procedure } = params.data;
    const restProcedure = getRestProcedure(router, procedure);
    if (!restProcedure)
        return res.status(404).json({ error: "not_found", error_description: "Unknown REST procedure." });
    ...
    if (!procedureType)
        return res.status(405).json({ error: "invalid_request", error_description: "Unsupported method." });

    const input = parseInput(req);
    if (input === null)
        return res.status(400).json({ error: "invalid_request", error_description: "Invalid input payload." });
    ...
    if (authContext.actor && restProcedure.scopes.length > 0) {
        ...
        if (!allowed)
            return res.status(403).json({ error: "forbidden", error_description: "Missing required scope." });
    }
    ...
    const target = (caller as Record<string, Record<string, (arg: unknown) => Promise<unknown>>>)[router]?.[procedure];

    if (!target)
        return res.status(404).json({ error: "not_found", error_description: "Unknown tRPC procedure." });
    ...
    try {
        ...
        return res.status(500).json({ error: "internal_error", error_description: "Failed to execute procedure." });
    }
}
